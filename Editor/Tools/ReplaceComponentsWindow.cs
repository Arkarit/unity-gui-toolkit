...
namespace GuiToolkit.Editor
{
	...
	public class ReplaceComponentsWindow : EditorWindow
	{
		...
		private void DoReplace()
		{
			...
			// Replace type name in C# source files so field/property/parameter declarations
			// referencing the old type are updated. The compiler will flag any incompatibilities.
			int csFilesModified = 0;
			string srcTypeName  = srcType.Name;
			string dstTypeName  = dstType.Name;
			var wordBoundary    = new Regex(@"\b" + Regex.Escape(srcTypeName) + @"\b");
			string[] csFiles    = Directory.GetFiles(Application.dataPath, "*.cs", SearchOption.AllDirectories);
			for (int i = 0; i < csFiles.Length; i++)
			{
				EditorUtility.DisplayProgressBar("Replace Components – C# files",
					Path.GetFileName(csFiles[i]), (float)i / csFiles.Length);
				string src  = File.ReadAllText(csFiles[i]);
				string dst  = wordBoundary.Replace(src, dstTypeName);
				if (dst != src)
				{
					File.WriteAllText(csFiles[i], dst);
					csFilesModified++;
				}
			}
			EditorUtility.ClearProgressBar();
			...
			string summary = $"Replaced: {componentsReplaced} '{srcType.Name}' component(s) → '{dstType.Name}'\n" +
			                 ...
			                 $"C# files:    {csFilesModified} modified";
			if (errors > 0)
