...
namespace GuiToolkit
{
	...
	public static class EditorUiUtility
	{
			...
			) where T : System.Enum
		{
			...
			using (new EditorGUILayout.HorizontalScope())
			{
				for (int i = 0, rectidx = rectIdxInitIdx; i < names.Length; i++)
				{
					...
					bool isMultiBit = EditorMiscUtility.MultipleBitsSet(currentEnumVal);
		...
		{
			if (!EditorObjectUtility.IsPrefab(_go))
				return false;
		...
		{
			if (!EditorMiscUtility.ValidateListAndIndex(_list, _idx))
				return;
		...
		{
			if (_idx1 == _idx2 || !EditorMiscUtility.ValidateListAndIndex(_list, _idx1) || !EditorMiscUtility.ValidateListAndIndex(_list, _idx2))
				return;
		...
		public static Color ColorPerEditorSkin(Color _lightSkin, Color _darkSkin) => EditorGUIUtility.isProSkin ? _darkSkin : _lightSkin;
