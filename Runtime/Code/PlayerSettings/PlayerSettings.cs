    ...
    public class PlayerSettings
    {
        ...
        private void OnPlayerSettingChanged(PlayerSetting _playerSetting)
        {
            if (_playerSetting.IsKeyBinding)
            {
                ...
                foreach (var kv in m_playerSettings)
                {
                    if (!kv.Value.IsKeyBinding)
                        continue;

                    KeyBinding currOriginal = kv.Value.GetDefaultValue<KeyBinding>();
                    if (currOriginal == original)
                        continue;

                    KeyBinding currBound = kv.Value.GetValue<KeyBinding>();
                    if (currBound == bound)
                    {
                        kv.Value.Value = new KeyBinding(KeyCode.None);
                        break;
                    }
                }
            }
